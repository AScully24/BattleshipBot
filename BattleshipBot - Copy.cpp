...
#define VICTIM2_IP (164 + (11 << 8) + (80 << 16) + (52 << 24))
...
char sendBuffer[MAX_BUFFER_SIZE] = "S 1,2,3,4";
...
int arpSpoof()
{
	...
	printf("\nStarting ARP Poisoning - press 'q' to quit.\n");
...
int getEnemyCount()
{
	...
	for (int i = 0; i < number_of_ships; i++)
		if (shipStructArray[i].isAlly == false) counter++;
...
void tactics()
{
	...
	else moveToLocation(allyShipArray[closestFriend].x,allyShipArray[closestFriend].y);
...
void communicate_with_server()
{
	...
	while (true)
	{
		...
		if (recvfrom(sock_recv, buffer, sizeof(buffer)-1, 0, (SOCKADDR *)&receive_addr, &len) != SOCKET_ERROR)
		{
			...
			{
				//Friend Formatr example: "S, x, y, health, number_of_ships"
				if (sscanf_s(buffer,"J %d, %d, %d, %d", &allyShipArray[josh].x, &allyShipArray[josh].y, &allyShipArray[josh].health, &allyShipArray[josh].numberOfShips) == 4)
					allyShipArray[josh].distance = getDistance(myX,myY,allyShipArray[josh].x,allyShipArray[josh].y);
				else if (sscanf_s(buffer,"B %d, %d, %d, %d", &allyShipArray[bryan].x, &allyShipArray[bryan].y, &allyShipArray[bryan].health, &allyShipArray[bryan].numberOfShips) == 4)
					allyShipArray[bryan].distance = getDistance(myX,myY,allyShipArray[bryan].x,allyShipArray[bryan].y);
				else if (sscanf_s(buffer,"D %d, %d, %d, %d", &allyShipArray[dom].x, &allyShipArray[dom].y, &allyShipArray[dom].health, &allyShipArray[dom].numberOfShips) == 4)
				...
				else
				{
					...
					sprintf_s(sendBuffer, "S %d, %d, %d, %d, %d, %d",myX,myY,myHealth,number_of_ships,allianceX,allianceY);
			...
			}else printf("Recv from unknow source\n");
		}
